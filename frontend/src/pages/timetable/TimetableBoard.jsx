...
import { fetchTimetableByClass, saveTimetableBatch } from '../../services/api'
...
function TimetableBoard({ className, onBack }) {
  ...
  const getLocalSpan = (slotKey, startTime, endTime) => {
    ...
    const span = Math.round((new Date(`1970-01-01T${endTime}`) - new Date(`1970-01-01T${startTime}`)) / (45 * 60 * 1000))
  ...
  const updateLocalCell = (day, slotKey, cellData) => {
    setTimetable((prev) => {
      ...
      dayCells[slotKey] = { ...(dayCells[slotKey] || {}), ...cellData }
      next[day] = dayCells
      return next
    })
  }
  ...
  const handleStageSave = (id, payload, isNew, context) => {
    ...
    if (isNew) {
      ...
      setPendingCreates((prev) => ({ ...prev, [tempId]: normalizedPayload }))
    ...
    if (isTemp) {
      setPendingCreates((prev) => ({ ...prev, [id]: normalizedPayload }))
    } else {
      setPendingUpdates((prev) => ({ ...prev, [id]: { id, ...normalizedPayload } }))
  ...
  return (
    <div className="tt-container">
      {notification && (
        <div className={`tt-notification tt-notification-${notification.type}`}>
          {notification.msg}
        </div>
      )}

      <button className="tt-back-btn" onClick={onBack}>Back to Classes</button>

      <div className="tt-header">
        <h2>Timetable - {className}</h2>
        <p className="tt-hint">Click any cell to stage create, update, or delete locally</p>
      </div>

      {hasPendingChanges && (
        <div className="tt-save-banner">
          <div>
            Unsaved: {Object.keys(pendingCreates).length} create, {Object.keys(pendingUpdates).length} update, {Object.keys(pendingDeletes).length} delete
          </div>
          <button className="tt-btn tt-modal-save" onClick={handleSaveAll} disabled={savingAll}>
        ...
        <table className="tt-table">
          <thead>
            <tr>
              ...
              {SLOT_CONFIG.map((slot) => (
                <th key={slot.key} className={slot.type === 'period' ? 'tt-period-col' : 'tt-break-col'}>
                  {slot.type === 'period' ? (<>{slot.label}<br /><span>{slot.timeLabel}</span></>) : slot.label}
                </th>
              ))}
            </tr>
          </thead>
          ...
          <tbody>
            {days.map((day) => {
              ...
              SLOT_CONFIG.forEach((slot) => {
                ...
                cells.push(
                  <td
                    key={slot.key}
                    colSpan={colSpan}
                    className={`tt-cell ${isEmpty ? 'tt-cell-empty' : ''} ${isLab ? 'tt-cell-lab' : ''}`}
                    onClick={() => setEditTarget({ cell: cell || null, day: day.id, slotKey: slot.key, slot })}
                  >
                    {cell ? (
                      <>
                        <div className="tt-cell-subject">{cell.subject}</div>
                        {cell.staff && <div className="tt-cell-staff">{cell.staff}</div>}
                        {String(cell.id).startsWith('temp-') && <div className="tt-cell-dirty">unsaved</div>}
                      </>
                    ) : (
                      <div className="tt-plus">{isBreakOrLunchSlot ? 'Set' : '+'}</div>
                    )}
                  </td>
                )
              })
              ...
              return (
                <tr key={day.id}>
                  <td className="tt-day-cell">{day.name}</td>
                  {cells}
                </tr>
              )
            })}
